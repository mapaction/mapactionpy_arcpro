...
class TestMapChef(TestCase):
    ...
    def setUp(self):
        ...
        self.my_mxd_fpath = os.path.join(self.parent_dir, 'tests', 'test_data','arcgis_10_6_reference_landscape_bottom.mxd')
        self.my_aprx_fpath = os.path.join(self.parent_dir, 'tests', 'test_data','pro-2.5_eference_landscape_bottom.aprx')
        self.my_pagx_fpath = os.path.join(self.parent_dir, 'tests', 'test_data','pro-2.5_reference_portrait_bottom.pagx')
    ...
    def test_map_chef_constructor(self):
        my_aprx = arcpy.mp.ArcGISProject(self.my_aprx_fpath)
        my_aprx.importDocument(self.my_mxd_fpath)
    ...
    def test_map_chef_cook(self):
        my_aprx = arcpy.mp.ArcGISProject(self.my_aprx_fpath)
        my_aprx.importDocument(self.my_pagx_fpath)
    ...
    def test_apply_frame_crs_and_extent(self):
        ...
        # my_mxd = arcpy.mp.MapDocument(self.my_mxd_fpath)
        # arc_frame = arcpy.mapping.ListDataFrames(my_mxd, recipe.principal_map_frame).pop()

        my_aprx = arcpy.mp.ArcGISProject(self.my_aprx_fpath)
        my_aprx.importDocument(self.my_pagx_fpath)
        arc_frame = my_aprx.listMaps("Main map")[0]
    ...
    def test_apply_definition_query(self):
        ...
        # https://desktop.arcgis.com/en/arcpro/10.6/tools/data-management-toolbox/get-count.htm
