import mapactionpy_arcpro.arcpro_runner as arcpro_runner
import os
import six
import sys
...
class TestArcProRunner(TestCase):
    ...
    def setUp(self):
        ...
        self.arcpro_runner = arcpro_runner.ArcProRunner(self.event)
    ...
    # @mock.patch('arcpy.mp.ArcGISProject')
    # @mock.patch('arcpy.mp.ArcGISProject')
    ...
    #     mock_MapDocument.my_pagx_fpath = os.path.join(self.parent_dir, 'tests', 'test_data','pro-2.5_eference_landscape_bottom.aprx')
    ...
    #     possible_templates = self._get_all_templates_by_regex(fixtures.fixture_recipe_minimal)

    #     actual_result = self.arcpro_runner.get_aspect_ratios_of_templates(possible_templates, test_recipe)

    #     self.assertEqual(actual_result, expected_result)

    @mock.patch('arcpy.mp.ArcGISProject.ExportToJPEG')
    @mock.patch('arcpy.mp.ArcGISProject.exportToPDF')
    @mock.patch('mapactionpy_arcpro.arcpro_runner.ArcProRunner.export_png_thumbnail')
    @mock.patch('arcpy.mp.ArcGISProject')
    @mock.patch('mapactionpy_arcpro.arcpro_runner.os.path.getsize')
    @mock.patch('mapactionpy_arcpro.arcpro_runner.get_map_scale')
    @mock.patch('mapactionpy_arcpro.arcpro_runner.get_map_spatial_ref')
    def test_do_export_params(self, mock_jpeg, mock_pdf, mock_png, mock_mapdoc, mock_getsize,
        ...
        test_recipe.map_project_path = os.path.join(
            self.parent_dir, 'tests', 'test_data', 'pro-2.5_eference_landscape_bottom.aprx')

        initial_export_params = {}
        initial_export_params["exportDirectory"] = os.path.join(
            self.parent_dir, 'tests', 'test_data', 'outputs')

        self.arcpro_runner._do_export(test_recipe)
    ...
    @skip('Not ready yet')
    def test_arcpro_runner_main_unknown_product(self):
        sys.argv[1:] = ['--eventConfigFile', os.path.join(self.cmf.path, 'event_description.json'),
                        '--template', os.path.join(self.cmf.map_templates,
